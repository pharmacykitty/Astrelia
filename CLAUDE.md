...
- **Phase 7 (first cut ✅):** **Galaxy Map mode** — `GalaxyMapView` renders the real local star field in 3D: HYG positions computed from RA/Dec/parallax → XYZ parsecs, Sun at origin. SwiftUI `Canvas` + a simd orbit camera (`lookAt`/`perspective` matrices); drag to orbit, pinch to zoom, tap-to-select (projected-nearest hit test), "Fly here" retargets the camera. Stars sized by absolute magnitude, coloured by B−V, depth-faded. Wired into the menu (replaces the placeholder). **Next:** stylized Milky Way / galactic-plane backdrop, exoplanets (NASA Exoplanet Archive) on "fly to a star", a larger distance-limited catalog (naked-eye subset is a local bubble), octree for picking/culling at scale, and the eventual move to a Metal renderer + logarithmic depth / camera-relative coords for whole-galaxy zoom.
