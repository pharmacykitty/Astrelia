...
- **Phase 5 (in progress):** Tap-to-detail astrophysical data sheets. **First cut ✅:** `CelestialCore.Astrophysics` (derived stellar radius via Stefan–Boltzmann, light-travel time, parsec↔ly; unit-tested) + `StarFacts` "relatable comparisons" that turn raw numbers into human-anchored sentences (light left in year Y, ×Sun brightness/heat/size). Surfaced in the Catalog `StarDetailView` (derived radius row + comparison callouts) and the Galaxy Map star card (light-travel line). Galaxy Map now shows a persistent **distance-from-Earth** readout. **Next:** HR-diagram "you are here" widget, size-comparison silhouette, planet/exoplanet detail sheets, time scrubber, tonight's-sky events.
