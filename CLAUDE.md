...
- **Phase 4 (✅ working):** Live AR "point at the sky" view, verified on-device. Full naked-eye star catalog (HYG mag ≤ 6.5, ~8.9k stars, bundled as `App/Resources/hyg_naked_eye.csv`, ~560 KB) drawn via `Canvas` (sized/coloured by magnitude & B−V), brightest stars labelled, Sun & Moon as nodes. Filter sheet (brightness/labels/Sun-Moon/below-horizon). Bottom panel guides to Sun & Moon only. Next: constellations, camera passthrough, planets, polish, perf (cache star directions if pushing past mag ~5.5).
